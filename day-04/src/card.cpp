...
card::card(std::string const & card)
{

    // split the card strings into all 3 parts: card, winning, hands
...
int card::extractID(std::string const & card)
{
...
std::vector<int> card::extractHands(std::string const & hands)
{
...
std::vector<std::string> card::split(const std::string& s,
                                              char delimiter)
{
...
void card::calculatePoints()
{
    ...
    points = power(2, match - 1);
}

int card::power(int base, int exponent) const {
