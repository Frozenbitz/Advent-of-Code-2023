...
cubeConundrum::cubeConundrum(std::string const& singleLineOfText)
    : cumulativeGameData{
          ...
          {"red", -1},
          {"green", -1},
          {"blue", -1} // token should map to the colors used!
      }
{
    std::vector<token> gamesToken =
        tokenizeColorRangeFromString(singleLineOfText);
    tokenMax(gamesToken);
}

std::vector<token>
cubeConundrum::tokenizeColorRangeFromString(const std::string& lineOfText)
{
...
std::vector<std::string> cubeConundrum::split(const std::string& s,
                                              char delimiter)
{
...
int cubeConundrum::evaluatePower() const
{
    int returnVal = getCumulativeGameData("red") *
                    getCumulativeGameData("blue") *
                    getCumulativeGameData("green");

    if (returnVal < 0)
        throw std::invalid_argument("cannot be negative here");
